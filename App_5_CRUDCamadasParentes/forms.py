...
class FormAddParente(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.entrySexo = BuilderCombobox(self, "Sexo", ["Homem", "Mulher", "Outro"])
        self.entrySexo.pack(side='top', padx=self.padx, pady=self.pady, fill='x')
...
class FormEditParente(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.id = BuilderEntry(self, "ID", 5)
        self.id.pack(side='top', anchor='ne', padx=self.padx, pady=self.pady)
        ...
        self.entrySexo = BuilderCombobox(self, "Sexo", ["Homem", "Mulher", "Outro"])
        self.entrySexo.pack(side='top', padx=self.padx, pady=self.pady)
    ...
    def atualizarInformacoes(self, event):
        ...
        id = self.id.get()
        print(f"id = {id}")
        infocliente = DAL.Dados.ImportarLinhaBDParentesDesconectado(id)

        if infocliente is not None:
            print(f"linha encontrada!:")
            infocliente = list(infocliente)
            self.entryNome.entryVar.set(infocliente[1])
            self.entrySexo.comboboxVar.set(infocliente[2])
            self.entryIdade.entryVar.set(infocliente[3])
            self.entrySalario.entryVar.set(infocliente[4])
            self.entryFavorito.entryVar.set(infocliente[5])
        else:
